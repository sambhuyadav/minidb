...
* ``value`` is the heap RID (page_id, slot) the key maps to.

This index maps primary-key -> RID. It is rebuilt from the heap on startup
(see engine.py), which keeps the tree decoupled from the pager while still
demonstrating real B+ tree mechanics: search path, node splits on insert, and
borrow/merge on delete.
